...
public:
    ...
    std::vector<std::unique_ptr<Node>> parse() {
        ...
        while (!is_at_end()) {
            if (check(Token::Text)) {
                nodes.push_back(std::make_unique<TextNode>(advance().value));
   ...
   std::unique_ptr<Node> parse_block() {
    ...
    std::unique_ptr<Node> parse_macro() {
        ...
        while (!is_at_end()) {
             ...
             if (check(Token::Text)) body.push_back(std::make_unique<TextNode>(advance().value));
             else if (check(Token::ExpressionStart)) body.push_back(parse_print());
             else if (check(Token::BlockStart)) body.push_back(parse_block());
             else advance();
        }
        return std::make_unique<MacroNode>(std::move(name), std::move(args), std::move(body));
    }
    ...
    std::unique_ptr<Node> parse_set() {
        ...
        return std::make_unique<SetNode>(std::move(target), std::move(value));
    }
    ...
    std::unique_ptr<Node> parse_if() {
        ...
        while (!is_at_end()) {
             ...
             if (check(Token::Text)) {
                current_body->push_back(std::make_unique<TextNode>(advance().value));
            } else if (check(Token::ExpressionStart)) {
        ...
        return std::make_unique<IfNode>(std::move(condition), std::move(true_body), std::move(false_body));
    }
    ...
    std::unique_ptr<Node> parse_for() {
        ...
        while (!is_at_end()) {
            ...
            if (check(Token::Text)) {
                body.push_back(std::make_unique<TextNode>(advance().value));
            } else if (check(Token::ExpressionStart)) {
        ...
        return std::make_unique<ForStmt>(std::move(vars), std::move(iterable), std::move(body), std::move(filter_expr));
    }
    ...
    std::unique_ptr<Expr> parse_expression(bool allow_ternary = true) {
        ...
        if (allow_ternary && check(Token::Identifier) && peek().value == "if") {
            ...
            if (check(Token::Identifier) && peek().value == "else") {
                ...
                return std::make_unique<TernaryExpr>(std::move(cond), std::move(expr), std::move(else_expr));
            } else {
    ...
    std::unique_ptr<Expr> parse_compare() {
        ...
        while ((check(Token::Operator) && (peek().value == "==" || peek().value == "!=" || peek().value == "<" || peek().value == ">" || peek().value == "<=" || peek().value == ">=")) ||
            ...
            if (op == "is") {
                ...
                left = std::make_unique<TestExpr>(std::move(left), test_name, is_not);
            } else if (op == "not") {
                if (check(Token::Identifier) && peek().value == "in") {
                    advance(); // eat in
                    auto right = parse_add();
                    left = std::make_unique<BinaryExpr>("not in", std::move(left), std::move(right));
                } else {
    ...
    std::unique_ptr<Expr> parse_filter() {
        ...
        while (check(Token::Operator) && peek().value == "|") {
            ...
            if (check(Token::Identifier)) {
                ...
                left = std::make_unique<FilterExpr>(std::move(left), name, std::move(args));
            }
        }
        return left;
    }
    ...
    std::unique_ptr<Expr> parse_unary() {
        if (check(Token::Operator) && (peek().value == "+" || peek().value == "-")) {
            ...
            auto right = parse_unary();
             ...
             return std::make_unique<BinaryExpr>(op, std::make_unique<LiteralExpr>(0), std::move(right));
        }
    ...
    std::unique_ptr<Expr> parse_primary() {
        ...
        if (check(Token::Punctuation) && peek().value == "{") {
             ...
             if (check(Token::Punctuation) && peek().value == "}") advance();
             return std::make_unique<ObjectExpr>(std::move(items));
        }
        ...
        if (check(Token::Identifier)) {
            std::string name = advance().value;
             ...
             if (name == "none") return std::make_unique<LiteralExpr>(nullptr);
            ...
            if (check(Token::Punctuation) && peek().value == "(") {
                ...
                expr = std::make_unique<VarExpr>(name);
            }
            ...
            while (true) { // Chain of accesses
                if (check(Token::Punctuation) && peek().value == "[") {
                     ...
                     if (is_slice) {
                         auto slice_expr = std::make_unique<SliceExpr>(std::move(start), std::move(stop), std::move(step));
                         expr = std::make_unique<GetItemExpr>(std::move(expr), std::move(slice_expr));
                     } else {
                         // Normal index
                         // if start is null effectively (e.g. []) - invalid in Jinja usually but maybe empty key used?
                         if (!start) start = std::make_unique<LiteralExpr>("");
                         expr = std::make_unique<GetItemExpr>(std::move(expr), std::move(start));
                     }
                    ...
                    if (check(Token::Identifier)) {
                        ...
                        if (check(Token::Punctuation) && peek().value == "(") {
                             ...
                             if (check(Token::Punctuation) && peek().value == ")") advance();
                             expr = std::make_unique<MethodCallExpr>(std::move(expr), member, std::move(method_args));
                        } else {
                            expr = std::make_unique<GetAttrExpr>(std::move(expr), member);
                        }
                    }
                } else {
                    break;
            ...
            std::string val = advance().value;
             try {
                if (val.find('.') != std::string::npos) {
                    return std::make_unique<LiteralExpr>(std::stof(val));
                }
                return std::make_unique<LiteralExpr>(std::stoi(val));
             } catch (...) {
